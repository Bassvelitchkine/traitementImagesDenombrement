...
def open_photo1():
    ...
    #On ajoute les curseurs à l'intérieur des LabelFrame() qu'on vient de créer
    threshold1 = DoubleVar()
    scale11 = Scale(param11, variable=threshold1)
    scale11.pack()
    
    kernel_size1 = DoubleVar()
    scale12 = Scale(param12, variable=kernel_size1)
    scale12.pack()
    
    sigma1 = DoubleVar()
    scale13 = Scale(param13, variable=sigma1)
    scale13.pack()
    
    erosion_size1 = DoubleVar()
    scale14 = Scale(param14, variable=erosion_size1)
    scale14.pack()
    
    filtering_radius1 = DoubleVar()
    scale15 = Scale(param15, variable=filtering_radius1)
    scale15.pack()
    ...
    croix1=IntVar()
    bouton1 = Checkbutton(case1, text="Afficher les croix", variable=croix1, onvalue=1, offvalue=0)
    bouton1.pack()
...
def open_photo2():
    ...
    #On ajoute les curseurs à l'intérieur des LabelFrame() qu'on vient de créer
    threshold2 = DoubleVar()
    threshold2.set(0)
    scale21 = Scale(param21, variable=threshold2)
    scale21.pack()
    
    kernel_size2 = DoubleVar()
    kernel_size2.set(0)
    scale22 = Scale(param22, variable=kernel_size2)
    scale22.pack()
    
    sigma2 = DoubleVar()
    sigma2.set(0)
    scale23 = Scale(param23, variable=sigma2)
    scale23.pack()
    
    erosion_size2 = DoubleVar()
    erosion_size2.set(0)
    scale24 = Scale(param24, variable=erosion_size2)
    scale24.pack()
    
    filtering_radius2 = DoubleVar()
    filtering_radius2.set(0)
    scale25 = Scale(param25, variable=filtering_radius2)
    scale25.pack()
    
    #On créé également un bouton qui permettra de demander à l'utilisateur s'il souhaite qu'on affiche les croix rouges ou non
    croix2=IntVar()
    croix2.set(0)
    bouton2 = Checkbutton(case2, text="Afficher les croix",variable=croix2, onvalue=1, offvalue=0)
    bouton2.pack()
...
def photo_processing(image_entree,numero_photo):
    ...
    if numero_photo==1:
        ...
        image=cv2.GaussianBlur(image_entree,(kernel1,kernel1),sig1)
        ...
        T,image= cv2.threshold(image, thresh1, 0, type=cv2.THRESH_TOZERO)
            ...
            for (j,i) in liste_centres_1:
                if j<len(image)-11 and j>10 and i>10 and i<len(image[0])-11: #il ne faut pas dépasser les bords de l'image
                    for k in range(-10,10):
                        image[j+k,i+k]=[255,0,0]
                        image[j+k,i-k]=[255,0,0]
                        #Les deux lignes qui suivent permettent d'épaissir les traits de nos croix
                        image[j+k,i+k+1]=[255,0,0]
                        image[j+k,i-k-1]=[255,0,0]
                
    #Si la photo à traiter est la photo 2 
    elif numero_photo==2:
        #On applique un filtre gaussien à l'image, cf. internet pour comprendre ce que c'est
        image=cv2.GaussianBlur(image_entree,(kernel2,kernel2),sig2)
        
        #Il faut aussi appliquer un seuil pour éliminer tout le bruit. En effet, les petits points blancs qui constituent le bruit pourraient être comptés comme les objets que nous tentons de dénombrer
        T,image= cv2.threshold(image, thresh2, 0, type=cv2.THRESH_TOZERO)
...
def actualiser():
    ...
    #On ramène les valeurs des paramètres dans leurs ensembles de définition
    thresh1=int(thresh1/100*255)    
    kernel1=int(kernel1/100*15)    
    #kernel1 et kernel2 prennent des valeurs entières impaires
    if kernel1%2==0:
        kernel1+=1
    sig1=int(sig1/100*30)    
    erosion1=int(erosion1/100*15)
    #erosion1 et erosion2 prennent des valeurs entières impaires
    if erosion1%2==0:
        erosion1+=1    
    radius1=radius1//2
    X1=croix1.get()
    ...
    if (thresh2,kernel2,sigma2,erosion2,radius2)==(0,0,0,0,0):
        ...
        #Dans le cas contraire, on ramène les valeurs des paramètres de la photo 2 dans leurs ensembles de définition
        thresh2=int(thresh2/100*255)
        kernel2=int(kernel2/100*15)
        if kernel2%2==0:
            kernel2+=1    
        sig2=int(sig2/100*30)
        erosion2=int(erosion2/100*15)
        if erosion2%2==0:
            erosion2+=1
        radius2=radius2//2
...
def resultats():
    ...
    nombre_immobiles=0
    for x1,y1 in liste_centres_1:
        indice=0
        x2,y2=liste_centres_2[0]
        while (abs(x1-x2)>1 or abs(y1-y2)>1) and indice<len(liste_centres_2): #On considèrera qu'un objet n'a pas bougé s'il se trouve dans un rayon de quelques pixels autour de sa position initiale (le fait que les images soient traitées légèrement différemment apporte cette incertitude)
            x2,y2=liste_centres_2[indice]
            indice+=1
        if indice<len(liste_centres_2):
            nombre_immobiles+=1
    
    nombres_objets=[len(liste_centres_1),len(liste_centres_2)]
